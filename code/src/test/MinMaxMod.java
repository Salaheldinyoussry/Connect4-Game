...
public class MinMaxMod {
	...
	int noOfExandedNodes=0;
	public String getNextMove(String state, int depth,Boolean alphaBeta) {
		String newS =  max(state,-999999999,999999999,depth,  alphaBeta,"1").state;
		System.out.println("No of expanded Nodes: "+noOfExandedNodes);
		noOfExandedNodes=0;
		
		return newS;
	}
	
	private Pair min(String x , double a , double b, int depth ,Boolean alphaBeta , String nodeOrder) {
		  printNode(x,nodeOrder);   
		  noOfExandedNodes++;
		if(depth ==0) {
		   ...
		   for( String move : moves) {
			   Pair p = max(move , a ,b , depth-1 , alphaBeta,nodeOrder+","+i );
			   if(p.val<curr.val) {
				   curr.state=move;
				   curr.val=p.val;
			   }
	...
	private Pair max(String x , double a , double b, int depth , Boolean alphaBeta , String nodeOrder) {
		noOfExandedNodes++;
		   printNode(x,nodeOrder);  
		...
		if(depth ==0) {
		   ...
		   List <String> moves = getMovesMod(x, '1');
		    ...
		    int i=1;
		   for( String move : moves) {
			   Pair p = min(move , a ,b , depth-1, alphaBeta, nodeOrder+","+i );
			   if(p.val>curr.val) {
				   curr.state=move;
				   curr.val=p.val;
			   }
	...
	private void printNode(String x, String nodeOrder) {
		String [] a = x.split("#");
